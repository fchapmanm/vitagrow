...
export default function FavoritesCard({ plant, onGrowingPress, onDeletePress }: FavoritesCardProps) {
  const navigation = useNavigation<any>();

  const handleGrowingGuide = () => {
    navigation.navigate('PlantGuidePreview', { plant }); // 🔹 ya no pasamos onStartGrowing
  };

  const handleGrowingPress = () => {
    onGrowingPress(plant);
  };
    ...
    Alert.alert(
      ...
      [
        { text: 'Cancel', style: 'cancel' },
        {
          ...
          style: 'destructive',
          onPress: () => {
            if (onDeletePress) onDeletePress(plant);
          },
        },
      ]
    );
    ...
    <View style={styles.card}>
      ...
      <View style={styles.content}>
        ...
        <View style={styles.buttons}>
          <TouchableOpacity style={styles.guideButton} onPress={handleGrowingGuide}>
            <Ionicons name="book-outline" size={16} color="#14532d" />
            <Text style={styles.guideButtonText}>Growing Guide</Text>
          </TouchableOpacity>

          <TouchableOpacity style={styles.growingButton} onPress={handleGrowingPress}>
            <Ionicons name="play-circle-outline" size={16} color="#ffffff" />
            <Text style={styles.growingButtonText}>Start Growing</Text>
          </TouchableOpacity>
        </View>
      </View>
    </View>
